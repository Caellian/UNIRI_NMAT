...
\begin{conditionbox}[određivanje koeficijenata kubičnog splinea]
    \begin{align}
        p_0(x_0) =& y_0\nonumber\\
...
Uvijeti \ref{cs_continuation}, \ref{cs_continuation_fd}, \ref{cs_continuation_sd} i \ref{cs_sd} dovode do linearnog sustava od $n+1$ jednadžbe s $n+1$ nepoznanicom, odnosno
...
\begin{examplebox}[uporaba kubičnog splinea]
...
\subsection{Greška kubičnog splinea}

Za kubični spline ocjena greške je
...
Drugim rječima, ravnomjernim povečavanjem čvorova kubičnog splinea tako da $h\to0$, maksimalna greška te interpolacije se približava 0.
